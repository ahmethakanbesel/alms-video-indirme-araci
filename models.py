...
class User:
    ...
    def get_enrolled_courses(self):
        response = get_web_page(
            self.main_url + "/Course/GetEnrolledCourses?displayMode=list"
        )
        course_list = []
        matches = re.findall(
            r'<a class="coursename" href="(.*?)" title=".*?">(.*?)</a>',
            response,
            re.MULTILINE,
        )
...
class Downloader:
    ...
    def add_to_queue(
        self, url, id="", file_name="", file_extension="mp4", file_path=DOWNLOAD_FOLDER
    ):
        i = 2
        ...
        self.download_queue.append(
            (url, id, file_path + file_name + "." + file_extension)
        )
    ...
    def download(self, url, file_path):
        ...
        progress_bar.set_description(f"{file_path}")
    ...
    def start_downloads(self):
        threads = [
            threading.Thread(
                name="worker/task",
                target=self.task,
                args=(video_data[0], video_data[1], video_data[2]),
            )
            for video_data in self.download_queue
        ]
...
class Video:
    ...
    def fetch_data(self):
        ...
        response = request.post(
            self.main_url + "/Video/ManageInteraction?id=" + self.id, headers=headers
        )
